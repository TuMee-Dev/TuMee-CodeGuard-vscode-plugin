/**
 * Regex-based fallback scope resolution for guard tags
...
import type { ScopeBoundary } from './treeSitterScopeResolver';
...
/**
 * Find delimited scope (braces, parens, etc.)
 */
function findDelimitedScope(
  lines: string[],
  startLine: number,
  openDelim: string,
  closeDelim: string
...
/**
 * Find linear scope (line-based)
 */
function findLinearScope(
  lines: string[],
  startLine: number,
  pattern: RegExp
): { start: number; end: number } | null {
